...
function renderLibrary() {
  ...
  library.forEach(item => {
    ...
    el.innerHTML = `<span class="icon">${icon}</span> <strong>#${library.indexOf(item)+1}</strong> - ${item.name}<div class="date">${item.date}</div><p>${preview}</p><div class="actions"><button class="open" onclick="openFile(${item.id})">Open</button><button class="delete" onclick="deleteLibraryItem(${item.id})">Delete</button></div>`;
    div.appendChild(el);
  });
}
...
function renderContacts() {
  ...
  contacts.forEach(c => {
    ...
    el.innerHTML = `<div><strong>${c.name}</strong><br><small>${c.phone || 'No number'}</small></div><div><button onclick="sendMessageToContact('${c.phone}', '${c.name}')">Send Message</button><button class="delete" onclick="deleteContact(${c.id})">Delete</button></div>`;
    div.appendChild(el);
  });
}
...
function submitComplaintForm(e) {
  ...
  const id = Date.now();
  complaints.push({ id, subject, desc, date: new Date().toLocaleString('en-IN'), status: "Pending" });
  saveToStorage();
  renderComplaints();
  closeComplaintForm();
